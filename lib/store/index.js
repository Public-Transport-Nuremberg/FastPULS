...
class DB_Store {
        ...
        return new Promise((resolve, reject) => {
            ...
            const processNext = async () => {
                if (nextIndex < urls.length) {
                    ...
                    this.data[company][this.translation[company][response_json[`${company}Kennung`]]].Linien = [...new Set(line_numbers)];
    ...
    init = async () => {
        ...
        const vag_response = await fetch('https://start.vag.de/dm/api/v1/haltestellen/vag?name=Plärrer')
        ...
        for (let i = 0; i < vag_json.Haltestellen.length; i++) {
            ...
            this.setTranslation('VAG', station.VAGKennung, `${station.Haltestellenname}.${station.VAGKennung}`);
        }

        if (this.enableInitLog) process.log.info('Loading VGN Data');
        const vgn_startTime = Date.now();
        const vgn_response = await fetch('https://start.vag.de/dm/api/v1/haltestellen/vgn?name=Plärrer')
        ...
        if (this.enablePlus) {
            ...
            for (let key in this.data.VAG) {
                ...
                urls_vag.push(`https://start.vag.de/dm/api/v1/abfahrten/VAG/${station.VAGKennung}?timespan=1200&limitcount=5000`);
            };
            ...
            for (let key in this.data.VAG) {
                ...
                urls_vgn.push(`https://start.vag.de/dm/api/v1/abfahrten/VGN/${station.VGNKennung}?timespan=1200&limitcount=5000`);
            };
